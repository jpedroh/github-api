...
public class AppTest extends AbstractGitHubApiTestBase {
  ...
  @Test public void testCreateDeployment() throws IOException {
    GHRepository repository = getTestRepository();
    GHDeployment deployment = repository.createDeployment("master").payload("{\"user\":\"atmos\",\"room_id\":123456}").description("question").create();
    assertNotNull(deployment.getCreator());
    assertNotNull(deployment.getId());
  }

  @Test public void testListDeployments() throws IOException {
    GHRepository repository = getTestRepository();
  ...
  @Test public void testGetDeploymentStatuses() throws IOException {
    GHRepository repository = getTestRepository();
    GHDeployment deployment = repository.createDeployment("master").description("question").payload("{\"user\":\"atmos\",\"room_id\":123456}").create();
    GHDeploymentStatus ghDeploymentStatus = repository.createDeployStatus(deployment.getId(), GHDeploymentState.SUCCESS).description("success").targetUrl("http://www.github.com").create();
  ...
  private GHRepository getTestRepository() throws IOException {
    ...
    try {
      ...
      repository = gitHub.createRepository("github-api-test", "A test repository for testing" + "the github-api project", "http://github-api.kohsuke.org/", true);
      try {
        Thread.sleep(2000);
  ...
  @Test public void testListIssues() throws IOException {
    ...
    GHIssue homed = null;
    try {
      unhomed = repository.createIssue("testing").body("this is body").assignee(u).label("bug").label("question").create();
      assertEquals(unhomed.getNumber(), repository.getIssues(GHIssueState.OPEN, null).get(0).getNumber());
      homed = repository.createIssue("testing").body("this is body").assignee(u).label("bug").label("question").milestone(milestone).create();
  ...
  @Test public void testFetchPullRequest() throws Exception {
  ...
  @Test public void testFetchPullRequestAsList() throws Exception {
  ...
  @Test public void testRepoPermissions() throws Exception {
    kohsuke();
    GHRepository r = gitHub.getOrganization("jenkinsci").getRepository("jenkins");
  ...
  @Test public void testPublicKeys() throws Exception {
  ...
  @Test public void testBranches() throws Exception {
  ...
  @Test public void testPullRequestPopulate() throws Exception {
  ...
  @Test public void testCreateRelease() throws Exception {
    kohsuke();
  ...
  @Test public void testAddDeployKey() throws IOException {
  ...
  @Test public void testCommitStatusContext() throws IOException {
    GHRepository myRepository = getTestRepository();
  ...
  @Test public void testMemberPagenation() throws IOException {
  ...
  @Test public void testTrees() throws IOException {
  ...
  @Test public void checkToString() throws Exception {
  ...
  @Test public void listOrgMemberships() throws Exception {
    ...
    for (GHMembership m : me.listOrgMemberships()) {
      ...
      System.out.printf("%s %s %s\n", m.getOrganization().getLogin(), m.getState(), m.getRole());
    }
  }
