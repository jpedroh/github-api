...
public class GHRepositoryTest extends AbstractGitHubWireMockTest {
  ...
  @Test public void createSignedCommitVerifyError() throws IOException {
    GHRepository repository = getRepository();
    GHTree ghTree = new GHTreeBuilder(repository).textEntry("a", "", false).create();
    GHVerification verification = repository.createCommit().message("test signing").withSignature("-----BEGIN PGP SIGNATURE-----\ninvalid\n-----END PGP SIGNATURE-----").tree(ghTree.getSha()).create().getCommitShortInfo().getVerification();
    assertEquals(GPGVERIFY_ERROR, verification.getReason());
  }

  @Test public void createSignedCommitUnknownSignatureType() throws IOException {
    GHRepository repository = getRepository();
    GHTree ghTree = new GHTreeBuilder(repository).textEntry("a", "", false).create();
    GHVerification verification = repository.createCommit().message("test signing").withSignature("unknown").tree(ghTree.getSha()).create().getCommitShortInfo().getVerification();
  ...
  @Test public void getBranchNonExistentBut200Status() throws Exception {
    ...
    try {
      ...
      assertThat(e.getMessage(), equalTo("Server returned HTTP response code: 200, message: \'404 Not Found\' for URL: " + mockGitHub.apiServer().baseUrl() + "/repos/hub4j-test-org/github-api/branches/test/NonExistent"));
    }
  }
  ...
  @Test public void testUpdateRepository() throws Exception {
    ...
    GHRepository updated = builder.allowRebaseMerge(false).allowSquashMerge(false).deleteBranchOnMerge(true).description(description).downloads(false).downloads(false).homepage(homepage).issues(false).private_(true).projects(false).wiki(false).done();
  ...
  @Test public void addCollaborators() throws Exception {
    GHRepository repo = getRepository();
    GHUser user = getUser();
  ...
  @Test public void listCommitCommentsNoComments() throws IOException {
    List<GHCommitComment> commitComments = getRepository().listCommitComments("c413fc1e3057332b93850ea48202627d29a37de5").toList();
    assertThat("Commit has no comments", commitComments.isEmpty());
  }

  @Test public void listCommitCommentsSomeComments() throws IOException {
    List<GHCommitComment> commitComments = getRepository().listCommitComments("499d91f9f846b0087b2a20cf3648b49dc9c2eeef").toList();
  ...
  @Test public void markDown() throws Exception {
    ...
    String actual = IOUtils.toString(gitHub.getRepository("hub4j/github-api").renderMarkdown("@kohsuke to fix issue #1", MarkdownMode.GFM));
  ...
  @Test public void getDeleteBranchOnMerge() throws IOException {
    GHRepository r = getRepository();
    assertNotNull(r.isDeleteBranchOnMerge());
  }

  @Test public void setDeleteBranchOnMerge() throws IOException {
    GHRepository r = getRepository();
  ...
  @Test public void getRefsHeads() throws Exception {
    GHRepository repo = getTempRepository();
  ...
  @Test public void getRefsEmptyTags() throws Exception {
    GHRepository repo = getTempRepository();
    try {
      repo.getRefs("tags");
      fail();
      ...
      assertThat(e.getMessage(), containsString("{\"message\":\"Not Found\",\"documentation_url\":\"https://developer.github.com/v3/git/refs/#get-a-reference\"}"));
    }
  }

  @Test public void listRefs() throws Exception {
    GHRepository repo = getRepository();
    List<GHRef> ghRefs;
  ...
  @Test public void getRef() throws Exception {
    GHRepository repo = getRepository();
    GHRef ghRef;
  ...
  @Test public void listRefsHeads() throws Exception {
    GHRepository repo = getTempRepository();
  ...
  @Test public void listRefsEmptyTags() throws Exception {
    try {
      GHRepository repo = getTempRepository();
  ...
  @Test public void listTagsEmpty() throws Exception {
    GHRepository repo = getTempRepository();
  ...
  @Test public void listTags() throws Exception {
    GHRepository repo = getRepository();
  ...
  @Test public void checkWatchersCount() throws Exception {
    snapshotNotAllowed();
    GHRepository repo = getTempRepository();
  ...
  @Test public void checkStargazersCount() throws Exception {
    snapshotNotAllowed();
    GHRepository repo = getTempRepository();
  ...
  @Test public void listCollaborators() throws Exception {
    GHRepository repo = getRepository();
    List<GHUser> collaborators = repo.listCollaborators().toList();
    assertThat(collaborators.size(), greaterThan(10));
  }

  @Test public void listCollaboratorsFiltered() throws Exception {
    GHRepository repo = getRepository();
    List<GHUser> allCollaborators = repo.listCollaborators().toList();
    List<GHUser> filteredCollaborators = repo.listCollaborators(GHRepository.CollaboratorAffiliation.OUTSIDE).toList();
    assertThat(filteredCollaborators.size(), lessThan(allCollaborators.size()));
  }

  @Test public void getCheckRuns() throws Exception {
    final int expectedCount = 8;
    PagedIterable<GHCheckRun> checkRuns = gitHub.getOrganization("hub4j").getRepository("github-api").getCheckRuns("78b9ff49d47daaa158eb373c4e2e040f739df8b9");
  ...
  @Test public void getLastCommitStatus() throws Exception {
  ...
  @Test public void listStargazers() throws IOException {
    GHRepository repository = getRepository();
